...
import { createFeature, createLayer, MapDocument } from "../core";
...
// @ts-ignore
function createGeoJsonLayerCollectionAsAMapDocument(mapDocument: MapDocument) {
    return JSON.stringify({
        name: mapDocument.name,
        type: "LayerCollection",
        bbox: mapDocument.bbox,
        layers: mapDocument.layers.map((layer) => {
            return createGeoJsonFeatureCollectionAsALayer(layer);
        })
    });
}

function downloadGeoJsonFile(json: string, fileName: string) {
